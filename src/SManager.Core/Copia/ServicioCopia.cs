...
public sealed class ServicioCopia
{
        ...
        int idCopiador = 0)
    {
        ...
        var rutaCompletaDestino = ComparadorSincronizacion.ObtenerRutaDestino(par, infoArchivo.FullName);
        if (string.IsNullOrEmpty(rutaCompletaDestino))
        {
            return ResultadoCopia.Vacio;
        }
        ...
        if (!ComparadorSincronizacion.NecesitaCopia(infoArchivo, par))
        {
            return ResultadoCopia.Vacio;
        }

        var esNuevo = !File.Exists(rutaCompletaDestino);
        var rutaRaizOrigen = Path.GetFullPath(
